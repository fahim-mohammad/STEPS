...
const BUCKET = process.env.NEXT_PUBLIC_SUPABASE_BUCKET_INVESTMENT_PROOFS || 'investment-proofs'
...
export async function POST(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!investmentId) return NextResponse.json({ ok: false, error: 'Missing investment id' }, { status: 400 })
    ...
    const storagePath = `investment/${investmentId}/${fileId}.${safeExt}`
